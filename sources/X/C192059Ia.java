package X;

/* renamed from: X.9Ia  reason: invalid class name and case insensitive filesystem */
public final class C192059Ia extends C35201wU {
    public C192059Ia(AnonymousClass9IP r6, String str) {
        C56052rL A01 = C56052rL.A01();
        C1899593h.A1M(A01);
        C56052rL A0U = C1899593h.A0U();
        C56052rL.A0D(A0U, "action", "upi-unblock-vpa");
        if (C1899593h.A1Y(str, 0, false)) {
            C56052rL.A0D(A0U, "vpa", str);
        }
        C35201wU.A01(A0U, A01, r6, this);
    }
}
